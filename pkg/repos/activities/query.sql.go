...
func (q *Queries) GetActivitiesByAction(ctx context.Context, action string) ([]Activity, error) {
	return _GetActivitiesByAction(ctx, q.AsReadOnly(), action)
}

func (q *ReadOnlyQueries) GetActivitiesByAction(ctx context.Context, action string) ([]Activity, error) {
	return _GetActivitiesByAction(ctx, q, action)
}

func _GetActivitiesByAction(ctx context.Context, q CacheQuerierConn, action string) ([]Activity, error) {
	qctx, cancel := context.WithTimeout(ctx, time.Millisecond*250)
	defer cancel()
	q.GetConn().CountIntent("activities.GetActivitiesByAction")
	dbRead := func() (any, time.Duration, error) {
		cacheDuration := time.Duration(time.Millisecond * 60000)
		rows, err := q.GetConn().WQuery(qctx, "activities.GetActivitiesByAction", getActivitiesByAction, action)
		if err != nil {
	...
	var items []Activity
	err := q.GetCache().GetWithTtl(qctx, "activities:GetActivitiesByAction:"+hashIfLong(fmt.Sprintf("%+v", action)), &items, dbRead, false, false)
	if err != nil {
		return nil, err
	}
