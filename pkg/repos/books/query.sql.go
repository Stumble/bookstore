// Code generated by sqlc. DO NOT EDIT.
// versions:
//   sqlc v2.4.0-dev-wicked-fork
// source: query.sql

package books

import (
	"context"
	"crypto/sha256"
	"encoding/hex"
	"encoding/json"
	"fmt"
	"sync"
	"time"

	"github.com/jackc/pgx/v5"
	"github.com/rs/zerolog/log"
)

type BulkInsertByCopyfromParams struct {
	Name        string
	Description string
	Metadata    json.RawMessage
	Category    BookCategory
	Price       float32
}

const getAllBooks = `-- name: GetAllBooks :many
SELECT id, name, description, metadata, category, price, dummy_field, created_at, updated_at FROM books
`

// -- timeout : 250ms
// -- cache : 10m
func (q *Queries) GetAllBooks(ctx context.Context) ([]Book, error) {
	return _GetAllBooks(ctx, q.AsReadOnly())
}

func (q *ReadOnlyQueries) GetAllBooks(ctx context.Context) ([]Book, error) {
	return _GetAllBooks(ctx, q)
}

func _GetAllBooks(ctx context.Context, q CacheQuerierConn) ([]Book, error) {
	qctx, cancel := context.WithTimeout(ctx, time.Millisecond*250)
	defer cancel()
	q.GetConn().CountIntent("books.GetAllBooks")
	dbRead := func() (any, time.Duration, error) {
		cacheDuration := time.Duration(time.Millisecond * 600000)
		rows, err := q.GetConn().WQuery(qctx, "books.GetAllBooks", getAllBooks)
		if err != nil {
			return []Book(nil), 0, err
		}
		defer rows.Close()
		var items []Book
		for rows.Next() {
			var i *Book = new(Book)
			if err := rows.Scan(
				&i.ID,
				&i.Name,
				&i.Description,
				&i.Metadata,
				&i.Category,
				&i.Price,
				&i.DummyField,
				&i.CreatedAt,
				&i.UpdatedAt,
			); err != nil {
				return []Book(nil), 0, err
			}
			items = append(items, *i)
		}
		if err := rows.Err(); err != nil {
			return []Book(nil), 0, err
		}
		return items, cacheDuration, nil
	}
	if q.GetCache() == nil {
		items, _, err := dbRead()
		return items.([]Book), err
	}
	var items []Book
	err := q.GetCache().GetWithTtl(qctx, "books:GetAllBooks:", &items, dbRead, false, false)
	if err != nil {
		return nil, err
	}

	return items, err
}

const getAllBooks2 = `-- name: GetAllBooks2 :many
SELECT id, name, description, metadata, category, price, dummy_field, created_at, updated_at FROM books
`

// -- cache : 10m
// -- timeout : 1s
func (q *Queries) GetAllBooks2(ctx context.Context) ([]Book, error) {
	return _GetAllBooks2(ctx, q.AsReadOnly())
}

func (q *ReadOnlyQueries) GetAllBooks2(ctx context.Context) ([]Book, error) {
	return _GetAllBooks2(ctx, q)
}

func _GetAllBooks2(ctx context.Context, q CacheQuerierConn) ([]Book, error) {
	qctx, cancel := context.WithTimeout(ctx, time.Millisecond*1000)
	defer cancel()
	q.GetConn().CountIntent("books.GetAllBooks2")
	dbRead := func() (any, time.Duration, error) {
		cacheDuration := time.Duration(time.Millisecond * 600000)
		rows, err := q.GetConn().WQuery(qctx, "books.GetAllBooks2", getAllBooks2)
		if err != nil {
			return []Book(nil), 0, err
		}
		defer rows.Close()
		var items []Book
		for rows.Next() {
			var i *Book = new(Book)
			if err := rows.Scan(
				&i.ID,
				&i.Name,
				&i.Description,
				&i.Metadata,
				&i.Category,
				&i.Price,
				&i.DummyField,
				&i.CreatedAt,
				&i.UpdatedAt,
			); err != nil {
				return []Book(nil), 0, err
			}
			items = append(items, *i)
		}
		if err := rows.Err(); err != nil {
			return []Book(nil), 0, err
		}
		return items, cacheDuration, nil
	}
	if q.GetCache() == nil {
		items, _, err := dbRead()
		return items.([]Book), err
	}
	var items []Book
	err := q.GetCache().GetWithTtl(qctx, "books:GetAllBooks2:", &items, dbRead, false, false)
	if err != nil {
		return nil, err
	}

	return items, err
}

const getBookByID = `-- name: GetBookByID :one
SELECT id, name, description, metadata, category, price, dummy_field, created_at, updated_at FROM books WHERE id = $1
`

// -- timeout : 250ms
// -- cache : 10m
func (q *Queries) GetBookByID(ctx context.Context, id int32) (*Book, error) {
	return _GetBookByID(ctx, q.AsReadOnly(), id)
}

func (q *ReadOnlyQueries) GetBookByID(ctx context.Context, id int32) (*Book, error) {
	return _GetBookByID(ctx, q, id)
}

func _GetBookByID(ctx context.Context, q CacheQuerierConn, id int32) (*Book, error) {
	qctx, cancel := context.WithTimeout(ctx, time.Millisecond*250)
	defer cancel()
	q.GetConn().CountIntent("books.GetBookByID")
	dbRead := func() (any, time.Duration, error) {
		cacheDuration := time.Duration(time.Millisecond * 600000)
		row := q.GetConn().WQueryRow(qctx, "books.GetBookByID", getBookByID, id)
		var i *Book = new(Book)
		err := row.Scan(
			&i.ID,
			&i.Name,
			&i.Description,
			&i.Metadata,
			&i.Category,
			&i.Price,
			&i.DummyField,
			&i.CreatedAt,
			&i.UpdatedAt,
		)
		if err == pgx.ErrNoRows {
			return (*Book)(nil), cacheDuration, nil
		}
		return i, cacheDuration, err
	}
	if q.GetCache() == nil {
		i, _, err := dbRead()
		return i.(*Book), err
	}

	var i *Book
	err := q.GetCache().GetWithTtl(qctx, "books:GetBookByID:"+hashIfLong(fmt.Sprintf("%+v", id)), &i, dbRead, false, false)
	if err != nil {
		return nil, err
	}

	return i, err
}

const getBookByNameMaybe = `-- name: GetBookByNameMaybe :many
SELECT id, name, description, metadata, category, price, dummy_field, created_at, updated_at FROM books WHERE
  name LIKE coalesce($1, name)
`

// -- cache : 10m
// -- timeout : 1s
func (q *Queries) GetBookByNameMaybe(ctx context.Context, name *string) ([]Book, error) {
	return _GetBookByNameMaybe(ctx, q.AsReadOnly(), name)
}

func (q *ReadOnlyQueries) GetBookByNameMaybe(ctx context.Context, name *string) ([]Book, error) {
	return _GetBookByNameMaybe(ctx, q, name)
}

func _GetBookByNameMaybe(ctx context.Context, q CacheQuerierConn, name *string) ([]Book, error) {
	qctx, cancel := context.WithTimeout(ctx, time.Millisecond*1000)
	defer cancel()
	q.GetConn().CountIntent("books.GetBookByNameMaybe")
	dbRead := func() (any, time.Duration, error) {
		cacheDuration := time.Duration(time.Millisecond * 600000)
		rows, err := q.GetConn().WQuery(qctx, "books.GetBookByNameMaybe", getBookByNameMaybe, name)
		if err != nil {
			return []Book(nil), 0, err
		}
		defer rows.Close()
		var items []Book
		for rows.Next() {
			var i *Book = new(Book)
			if err := rows.Scan(
				&i.ID,
				&i.Name,
				&i.Description,
				&i.Metadata,
				&i.Category,
				&i.Price,
				&i.DummyField,
				&i.CreatedAt,
				&i.UpdatedAt,
			); err != nil {
				return []Book(nil), 0, err
			}
			items = append(items, *i)
		}
		if err := rows.Err(); err != nil {
			return []Book(nil), 0, err
		}
		return items, cacheDuration, nil
	}
	if q.GetCache() == nil {
		items, _, err := dbRead()
		return items.([]Book), err
	}
	var items []Book
	err := q.GetCache().GetWithTtl(qctx, "books:GetBookByNameMaybe:"+hashIfLong(fmt.Sprintf("%+v", ptrStr(name))), &items, dbRead, false, false)
	if err != nil {
		return nil, err
	}

	return items, err
}

const getBookBySpec = `-- name: GetBookBySpec :many
SELECT id, name, description, metadata, category, price, dummy_field, created_at, updated_at FROM books WHERE
  name LIKE coalesce($1, name) AND
  price = coalesce($2, price) AND
  ($3::int is NULL or dummy_field = $3)
`

type GetBookBySpecParams struct {
	Name  *string
	Price *float32
	Dummy *int32
}

// CacheKey - cache key
func (arg GetBookBySpecParams) CacheKey() string {
	prefix := "books:GetBookBySpec:"
	return prefix + hashIfLong(fmt.Sprintf("%+v,%+v,%+v", ptrStr(arg.Name), ptrStr(arg.Price), ptrStr(arg.Dummy)))
}

// -- timeout : 250ms
// -- cache : 10m
func (q *Queries) GetBookBySpec(ctx context.Context, arg GetBookBySpecParams) ([]Book, error) {
	return _GetBookBySpec(ctx, q.AsReadOnly(), arg)
}

func (q *ReadOnlyQueries) GetBookBySpec(ctx context.Context, arg GetBookBySpecParams) ([]Book, error) {
	return _GetBookBySpec(ctx, q, arg)
}

func _GetBookBySpec(ctx context.Context, q CacheQuerierConn, arg GetBookBySpecParams) ([]Book, error) {
	qctx, cancel := context.WithTimeout(ctx, time.Millisecond*250)
	defer cancel()
	q.GetConn().CountIntent("books.GetBookBySpec")
	dbRead := func() (any, time.Duration, error) {
		cacheDuration := time.Duration(time.Millisecond * 600000)
		rows, err := q.GetConn().WQuery(qctx, "books.GetBookBySpec", getBookBySpec, arg.Name, arg.Price, arg.Dummy)
		if err != nil {
			return []Book(nil), 0, err
		}
		defer rows.Close()
		var items []Book
		for rows.Next() {
			var i *Book = new(Book)
			if err := rows.Scan(
				&i.ID,
				&i.Name,
				&i.Description,
				&i.Metadata,
				&i.Category,
				&i.Price,
				&i.DummyField,
				&i.CreatedAt,
				&i.UpdatedAt,
			); err != nil {
				return []Book(nil), 0, err
			}
			items = append(items, *i)
		}
		if err := rows.Err(); err != nil {
			return []Book(nil), 0, err
		}
		return items, cacheDuration, nil
	}
	if q.GetCache() == nil {
		items, _, err := dbRead()
		return items.([]Book), err
	}
	var items []Book
	err := q.GetCache().GetWithTtl(qctx, arg.CacheKey(), &items, dbRead, false, false)
	if err != nil {
		return nil, err
	}

	return items, err
}

const insert = `-- name: Insert :exec
INSERT INTO books (
   name, description, metadata, category, price
) VALUES (
  $1, $2, $3, $4, $5
)
`

type InsertParams struct {
	Name        string
	Description string
	Metadata    json.RawMessage
	Category    BookCategory
	Price       float32
}

// -- invalidate : [GetAllBooks, GetAllBooks2]
// -- timeout : 100ms
func (q *Queries) Insert(ctx context.Context, arg InsertParams) error {
	qctx, cancel := context.WithTimeout(ctx, time.Millisecond*100)
	defer cancel()
	_, err := q.db.WExec(qctx, "books.Insert", insert,
		arg.Name,
		arg.Description,
		arg.Metadata,
		arg.Category,
		arg.Price)
	if err != nil {
		return err
	}
	// invalidate
	_ = q.db.PostExec(func() error {
		if q.cache == nil {
			return nil
		}
		anyErr := make(chan error, 2)
		var wg sync.WaitGroup
		wg.Add(2)
		go func() {
			defer wg.Done()
			key := "books:GetAllBooks:"
			invalidateErr := q.cache.Invalidate(ctx, key)
			if invalidateErr != nil {
				log.Ctx(ctx).Error().Err(invalidateErr).Msgf(
					"Failed to invalidate: %s", key)
				anyErr <- invalidateErr
			}
		}()
		go func() {
			defer wg.Done()
			key := "books:GetAllBooks2:"
			invalidateErr := q.cache.Invalidate(ctx, key)
			if invalidateErr != nil {
				log.Ctx(ctx).Error().Err(invalidateErr).Msgf(
					"Failed to invalidate: %s", key)
				anyErr <- invalidateErr
			}
		}()
		wg.Wait()
		close(anyErr)
		return <-anyErr
	})
	return nil
}

const insertAndReturnID = `-- name: InsertAndReturnID :one
INSERT INTO books (
   name, description, metadata, category, price
) VALUES (
  $1, $2, $3, $4, $5
) RETURNING id
`

type InsertAndReturnIDParams struct {
	Name        string
	Description string
	Metadata    json.RawMessage
	Category    BookCategory
	Price       float32
}

// -- timeout : 250ms
// -- invalidate : [GetAllBooks, GetAllBooks2]
func (q *Queries) InsertAndReturnID(ctx context.Context, arg InsertAndReturnIDParams) (*int32, error) {
	return _InsertAndReturnID(ctx, q, arg)
}

func _InsertAndReturnID(ctx context.Context, q CacheWGConn, arg InsertAndReturnIDParams) (*int32, error) {
	qctx, cancel := context.WithTimeout(ctx, time.Millisecond*250)
	defer cancel()
	row := q.GetConn().WQueryRow(qctx, "books.InsertAndReturnID", insertAndReturnID,
		arg.Name,
		arg.Description,
		arg.Metadata,
		arg.Category,
		arg.Price)
	var id *int32 = new(int32)
	err := row.Scan(id)
	if err == pgx.ErrNoRows {
		return (*int32)(nil), nil
	} else if err != nil {
		return nil, err
	}

	// invalidate
	_ = q.GetConn().PostExec(func() error {
		if q.GetCache() == nil {
			return nil
		}
		anyErr := make(chan error, 2)
		var wg sync.WaitGroup
		wg.Add(2)
		go func() {
			defer wg.Done()
			key := "books:GetAllBooks:"
			invalidateErr := q.GetCache().Invalidate(ctx, key)
			if invalidateErr != nil {
				log.Ctx(ctx).Error().Err(invalidateErr).Msgf(
					"Failed to invalidate: %s", key)
				anyErr <- invalidateErr
			}
		}()
		go func() {
			defer wg.Done()
			key := "books:GetAllBooks2:"
			invalidateErr := q.GetCache().Invalidate(ctx, key)
			if invalidateErr != nil {
				log.Ctx(ctx).Error().Err(invalidateErr).Msgf(
					"Failed to invalidate: %s", key)
				anyErr <- invalidateErr
			}
		}()
		wg.Wait()
		close(anyErr)
		return <-anyErr
	})
	return id, err
}

const insertReturnInvalidate = `-- name: InsertReturnInvalidate :one
INSERT INTO books (
   name, description, metadata, category, price
) VALUES (
  $1, $2, $3, $4, $5
) RETURNING id
`

type InsertReturnInvalidateParams struct {
	Name        string
	Description string
	Metadata    json.RawMessage
	Category    BookCategory
	Price       float32
}

// -- timeout : 250ms
// -- invalidate : [GetBookByID]
func (q *Queries) InsertReturnInvalidate(ctx context.Context, arg InsertReturnInvalidateParams, getBookByID *int32) (*int32, error) {
	return _InsertReturnInvalidate(ctx, q, arg, getBookByID)
}

func _InsertReturnInvalidate(ctx context.Context, q CacheWGConn, arg InsertReturnInvalidateParams, getBookByID *int32) (*int32, error) {
	qctx, cancel := context.WithTimeout(ctx, time.Millisecond*250)
	defer cancel()
	row := q.GetConn().WQueryRow(qctx, "books.InsertReturnInvalidate", insertReturnInvalidate,
		arg.Name,
		arg.Description,
		arg.Metadata,
		arg.Category,
		arg.Price)
	var id *int32 = new(int32)
	err := row.Scan(id)
	if err == pgx.ErrNoRows {
		return (*int32)(nil), nil
	} else if err != nil {
		return nil, err
	}

	// invalidate
	_ = q.GetConn().PostExec(func() error {
		if q.GetCache() == nil {
			return nil
		}
		anyErr := make(chan error, 1)
		var wg sync.WaitGroup
		wg.Add(1)
		go func() {
			defer wg.Done()
			if getBookByID != nil {
				key := "books:GetBookByID:" + hashIfLong(fmt.Sprintf("%+v", (*getBookByID)))
				invalidateErr := q.GetCache().Invalidate(ctx, key)
				if invalidateErr != nil {
					log.Ctx(ctx).Error().Err(invalidateErr).Msgf(
						"Failed to invalidate: %s", key)
					anyErr <- invalidateErr
				}
			}
		}()
		wg.Wait()
		close(anyErr)
		return <-anyErr
	})
	return id, err
}

const insertWithInvalidate = `-- name: InsertWithInvalidate :exec
INSERT INTO books (
   id, name, description, metadata, category, dummy_field, price
) VALUES (
  $1, $2, $3, $4, $5, $6, $7
)
`

type InsertWithInvalidateParams struct {
	ID          int32
	Name        string
	Description string
	Metadata    json.RawMessage
	Category    BookCategory
	DummyField  *int32
	Price       float32
}

// -- timeout : 250ms
// -- invalidate : [GetBookByNameMaybe, GetBookBySpec]
func (q *Queries) InsertWithInvalidate(ctx context.Context, arg InsertWithInvalidateParams, getBookByNameMaybe **string, getBookBySpec *GetBookBySpecParams) error {
	qctx, cancel := context.WithTimeout(ctx, time.Millisecond*250)
	defer cancel()
	_, err := q.db.WExec(qctx, "books.InsertWithInvalidate", insertWithInvalidate,
		arg.ID,
		arg.Name,
		arg.Description,
		arg.Metadata,
		arg.Category,
		arg.DummyField,
		arg.Price)
	if err != nil {
		return err
	}
	// invalidate
	_ = q.db.PostExec(func() error {
		if q.cache == nil {
			return nil
		}
		anyErr := make(chan error, 2)
		var wg sync.WaitGroup
		wg.Add(2)
		go func() {
			defer wg.Done()
			if getBookByNameMaybe != nil {
				key := "books:GetBookByNameMaybe:" + hashIfLong(fmt.Sprintf("%+v", ptrStr((*getBookByNameMaybe))))
				invalidateErr := q.cache.Invalidate(ctx, key)
				if invalidateErr != nil {
					log.Ctx(ctx).Error().Err(invalidateErr).Msgf(
						"Failed to invalidate: %s", key)
					anyErr <- invalidateErr
				}
			}
		}()
		go func() {
			defer wg.Done()
			if getBookBySpec != nil {
				key := (*getBookBySpec).CacheKey()
				invalidateErr := q.cache.Invalidate(ctx, key)
				if invalidateErr != nil {
					log.Ctx(ctx).Error().Err(invalidateErr).Msgf(
						"Failed to invalidate: %s", key)
					anyErr <- invalidateErr
				}
			}
		}()
		wg.Wait()
		close(anyErr)
		return <-anyErr
	})
	return nil
}

const listByCategory = `-- name: ListByCategory :many
SELECT id, name, description, metadata, category, price, dummy_field, created_at, updated_at
FROM
  books
WHERE
  category = $1 AND id > $2
ORDER BY
  id
LIMIT $3
`

type ListByCategoryParams struct {
	Category BookCategory
	After    int32
	First    int32
}

// -- timeout : 1.5s
func (q *Queries) ListByCategory(ctx context.Context, arg ListByCategoryParams) ([]Book, error) {
	return _ListByCategory(ctx, q.AsReadOnly(), arg)
}

func (q *ReadOnlyQueries) ListByCategory(ctx context.Context, arg ListByCategoryParams) ([]Book, error) {
	return _ListByCategory(ctx, q, arg)
}

func _ListByCategory(ctx context.Context, q CacheQuerierConn, arg ListByCategoryParams) ([]Book, error) {
	qctx, cancel := context.WithTimeout(ctx, time.Millisecond*1500)
	defer cancel()
	q.GetConn().CountIntent("books.ListByCategory")
	rows, err := q.GetConn().WQuery(qctx, "books.ListByCategory", listByCategory, arg.Category, arg.After, arg.First)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []Book
	for rows.Next() {
		var i *Book = new(Book)
		if err := rows.Scan(
			&i.ID,
			&i.Name,
			&i.Description,
			&i.Metadata,
			&i.Category,
			&i.Price,
			&i.DummyField,
			&i.CreatedAt,
			&i.UpdatedAt,
		); err != nil {
			return nil, err
		}
		items = append(items, *i)
	}
	if err := rows.Err(); err != nil {
		return nil, err
	}

	return items, err
}

const partialUpdateByID = `-- name: PartialUpdateByID :exec
UPDATE books
SET
  description = coalesce($1, description),
  metadata = coalesce($2, metadata),
  price = coalesce($3, price),
  dummy_field = coalesce($4, dummy_field),
  updated_at = NOW()
WHERE
  id = $5
`

type PartialUpdateByIDParams struct {
	Description *string
	Meta        json.RawMessage
	Price       *float32
	DummyField  *int32
	ID          int32
}

// -- timeout : 250ms
func (q *Queries) PartialUpdateByID(ctx context.Context, arg PartialUpdateByIDParams) error {
	qctx, cancel := context.WithTimeout(ctx, time.Millisecond*250)
	defer cancel()
	_, err := q.db.WExec(qctx, "books.PartialUpdateByID", partialUpdateByID,
		arg.Description,
		arg.Meta,
		arg.Price,
		arg.DummyField,
		arg.ID)
	if err != nil {
		return err
	}

	return nil
}

const refreshIDSerial = `-- name: RefreshIDSerial :exec
SELECT setval(seq_name, (SELECT MAX(id) FROM books)+1, false)
FROM PG_GET_SERIAL_SEQUENCE('books', 'id') as seq_name
`

// -- timeout : 300ms
func (q *Queries) RefreshIDSerial(ctx context.Context) error {
	qctx, cancel := context.WithTimeout(ctx, time.Millisecond*300)
	defer cancel()
	_, err := q.db.WExec(qctx, "books.RefreshIDSerial", refreshIDSerial)
	if err != nil {
		return err
	}

	return nil
}

const searchBooks = `-- name: SearchBooks :many
SELECT id, name, description, metadata, category, price, dummy_field, created_at, updated_at FROM books WHERE name like $1
`

// -- timeout : 250ms
// -- cache : 10m
func (q *Queries) SearchBooks(ctx context.Context, name string) ([]Book, error) {
	return _SearchBooks(ctx, q.AsReadOnly(), name)
}

func (q *ReadOnlyQueries) SearchBooks(ctx context.Context, name string) ([]Book, error) {
	return _SearchBooks(ctx, q, name)
}

func _SearchBooks(ctx context.Context, q CacheQuerierConn, name string) ([]Book, error) {
	qctx, cancel := context.WithTimeout(ctx, time.Millisecond*250)
	defer cancel()
	q.GetConn().CountIntent("books.SearchBooks")
	dbRead := func() (any, time.Duration, error) {
		cacheDuration := time.Duration(time.Millisecond * 600000)
		rows, err := q.GetConn().WQuery(qctx, "books.SearchBooks", searchBooks, name)
		if err != nil {
			return []Book(nil), 0, err
		}
		defer rows.Close()
		var items []Book
		for rows.Next() {
			var i *Book = new(Book)
			if err := rows.Scan(
				&i.ID,
				&i.Name,
				&i.Description,
				&i.Metadata,
				&i.Category,
				&i.Price,
				&i.DummyField,
				&i.CreatedAt,
				&i.UpdatedAt,
			); err != nil {
				return []Book(nil), 0, err
			}
			items = append(items, *i)
		}
		if err := rows.Err(); err != nil {
			return []Book(nil), 0, err
		}
		return items, cacheDuration, nil
	}
	if q.GetCache() == nil {
		items, _, err := dbRead()
		return items.([]Book), err
	}
	var items []Book
	err := q.GetCache().GetWithTtl(qctx, "books:SearchBooks:"+hashIfLong(fmt.Sprintf("%+v", name)), &items, dbRead, false, false)
	if err != nil {
		return nil, err
	}

	return items, err
}

const simpleCachedQuery = `-- name: SimpleCachedQuery :many
SELECT id, name, description, metadata, category, price, dummy_field, created_at, updated_at FROM books
`

// -- cache : 10m
// -- timeout : 250ms
func (q *Queries) SimpleCachedQuery(ctx context.Context) ([]Book, error) {
	return _SimpleCachedQuery(ctx, q.AsReadOnly())
}

func (q *ReadOnlyQueries) SimpleCachedQuery(ctx context.Context) ([]Book, error) {
	return _SimpleCachedQuery(ctx, q)
}

func _SimpleCachedQuery(ctx context.Context, q CacheQuerierConn) ([]Book, error) {
	qctx, cancel := context.WithTimeout(ctx, time.Millisecond*250)
	defer cancel()
	q.GetConn().CountIntent("books.SimpleCachedQuery")
	dbRead := func() (any, time.Duration, error) {
		cacheDuration := time.Duration(time.Millisecond * 600000)
		rows, err := q.GetConn().WQuery(qctx, "books.SimpleCachedQuery", simpleCachedQuery)
		if err != nil {
			return []Book(nil), 0, err
		}
		defer rows.Close()
		var items []Book
		for rows.Next() {
			var i *Book = new(Book)
			if err := rows.Scan(
				&i.ID,
				&i.Name,
				&i.Description,
				&i.Metadata,
				&i.Category,
				&i.Price,
				&i.DummyField,
				&i.CreatedAt,
				&i.UpdatedAt,
			); err != nil {
				return []Book(nil), 0, err
			}
			items = append(items, *i)
		}
		if err := rows.Err(); err != nil {
			return []Book(nil), 0, err
		}
		return items, cacheDuration, nil
	}
	if q.GetCache() == nil {
		items, _, err := dbRead()
		return items.([]Book), err
	}
	var items []Book
	err := q.GetCache().GetWithTtl(qctx, "books:SimpleCachedQuery:", &items, dbRead, false, false)
	if err != nil {
		return nil, err
	}

	return items, err
}

const updateBookByID = `-- name: UpdateBookByID :exec
UPDATE books
SET
  description = $1, metadata = $2, price = $3, updated_at = NOW()
WHERE
  id = $4
`

type UpdateBookByIDParams struct {
	Description string
	Meta        json.RawMessage
	Price       float32
	ID          int32
}

// -- invalidate : [GetBookByID]
// -- timeout : 1s
func (q *Queries) UpdateBookByID(ctx context.Context, arg UpdateBookByIDParams, getBookByID *int32) error {
	qctx, cancel := context.WithTimeout(ctx, time.Millisecond*1000)
	defer cancel()
	_, err := q.db.WExec(qctx, "books.UpdateBookByID", updateBookByID,
		arg.Description,
		arg.Meta,
		arg.Price,
		arg.ID)
	if err != nil {
		return err
	}
	// invalidate
	_ = q.db.PostExec(func() error {
		if q.cache == nil {
			return nil
		}
		anyErr := make(chan error, 1)
		var wg sync.WaitGroup
		wg.Add(1)
		go func() {
			defer wg.Done()
			if getBookByID != nil {
				key := "books:GetBookByID:" + hashIfLong(fmt.Sprintf("%+v", (*getBookByID)))
				invalidateErr := q.cache.Invalidate(ctx, key)
				if invalidateErr != nil {
					log.Ctx(ctx).Error().Err(invalidateErr).Msgf(
						"Failed to invalidate: %s", key)
					anyErr <- invalidateErr
				}
			}
		}()
		wg.Wait()
		close(anyErr)
		return <-anyErr
	})
	return nil
}

//// auto generated functions

func (q *Queries) Dump(ctx context.Context, beforeDump ...BeforeDump) ([]byte, error) {
	sql := "SELECT id,name,description,metadata,category,price,dummy_field,created_at,updated_at FROM \"books\" ORDER BY id,name,description,price,dummy_field,created_at,updated_at ASC;"
	rows, err := q.db.WQuery(ctx, "books.Dump", sql)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []Book
	for rows.Next() {
		var v Book
		if err := rows.Scan(&v.ID, &v.Name, &v.Description, &v.Metadata, &v.Category, &v.Price, &v.DummyField, &v.CreatedAt, &v.UpdatedAt); err != nil {
			return nil, err
		}
		for _, applyBeforeDump := range beforeDump {
			applyBeforeDump(&v)
		}
		items = append(items, v)
	}
	if err := rows.Err(); err != nil {
		return nil, err
	}
	bytes, err := json.MarshalIndent(items, "", "  ")
	if err != nil {
		return nil, err
	}
	return bytes, nil
}

func (q *Queries) Load(ctx context.Context, data []byte) error {
	sql := "INSERT INTO \"books\" (id,name,description,metadata,category,price,dummy_field,created_at,updated_at) VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9);"
	rows := make([]Book, 0)
	err := json.Unmarshal(data, &rows)
	if err != nil {
		return err
	}
	for _, row := range rows {
		_, err := q.db.WExec(ctx, "books.Load", sql, row.ID, row.Name, row.Description, row.Metadata, row.Category, row.Price, row.DummyField, row.CreatedAt, row.UpdatedAt)
		if err != nil {
			return err
		}
	}
	return nil
}

func hashIfLong(v string) string {
	if len(v) > 64 {
		hash := sha256.Sum256([]byte(v))
		return "h(" + hex.EncodeToString(hash[:]) + ")"
	}
	return v
}

func ptrStr[T any](v *T) string {
	if v == nil {
		return "<nil>"
	}
	return fmt.Sprintf("%+v", *v)
}

// eliminate unused error
var _ = log.Logger
var _ = fmt.Sprintf("")
var _ = time.Now()
var _ = json.RawMessage{}
var _ = sha256.Sum256(nil)
var _ = hex.EncodeToString(nil)
var _ = sync.WaitGroup{}
