...
func (q *Queries) PartialUpdateByID(ctx context.Context, arg PartialUpdateByIDParams, getBookByID *int64) error {
	_, err := q.db.WExec(ctx, "PartialUpdateByID", partialUpdateByID,
		arg.Description,
		arg.Meta,
		arg.Price,
		arg.DummyField,
...
func (q *Queries) Dump(ctx context.Context, beforeDump ...BeforeDump) ([]byte, error) {
	sql := "SELECT id,name,description,metadata,category,price,dummy_field,created_at,updated_at FROM books ORDER BY id,name,description,metadata,category,price,dummy_field,created_at,updated_at ASC;"
...
func (q *Queries) Load(ctx context.Context, data []byte) error {
	...
	for _, row := range rows {
		_, err := q.db.WExec(ctx, "Load", sql, row.ID, row.Name, row.Description, row.Metadata, row.Category, row.Price, row.DummyField, row.CreatedAt, row.UpdatedAt)
		if err != nil {
			return err
		}
	}
