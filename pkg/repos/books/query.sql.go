...
func (q *Queries) GetAllBooks(ctx context.Context) ([]Book, error) {
	...
	dbRead := func() (any, time.Duration, error) {
		...
		rows, err := q.db.WQuery(qctx, "books.GetAllBooks", getAllBooks)
		if err != nil {
	...
	var items []Book
	err := q.cache.GetWithTtl(qctx, "books:GetAllBooks:", &items, dbRead, false, false)
	if err != nil {
		return nil, err
	}
...
func (q *Queries) GetAllBooks2(ctx context.Context) ([]Book, error) {
	...
	dbRead := func() (any, time.Duration, error) {
		cacheDuration := time.Duration(time.Millisecond * 600000)
		rows, err := q.db.WQuery(qctx, "books.GetAllBooks2", getAllBooks2)
		if err != nil {
			return []Book(nil), 0, err
		}
		defer rows.Close()
	...
	var items []Book
	err := q.cache.GetWithTtl(qctx, "books:GetAllBooks2:", &items, dbRead, false, false)
	if err != nil {
		return nil, err
	}
...
func (q *Queries) GetBookByID(ctx context.Context, id int32) (*Book, error) {
	...
	dbRead := func() (any, time.Duration, error) {
		cacheDuration := time.Duration(time.Millisecond * 600000)
		row := q.db.WQueryRow(qctx, "books.GetBookByID", getBookByID, id)
		var i *Book = new(Book)
		err := row.Scan(
			&i.ID,
			&i.Name,
	...
	var i *Book
	err := q.cache.GetWithTtl(qctx, "books:GetBookByID:"+hashIfLong(fmt.Sprintf("%+v", id)), &i, dbRead, false, false)
	if err != nil {
		return nil, err
	}
...
func (q *Queries) GetBookByNameMaybe(ctx context.Context, name *string) ([]Book, error) {
	...
	dbRead := func() (any, time.Duration, error) {
		cacheDuration := time.Duration(time.Millisecond * 600000)
		rows, err := q.db.WQuery(qctx, "books.GetBookByNameMaybe", getBookByNameMaybe, name)
		if err != nil {
			return []Book(nil), 0, err
		}
		defer rows.Close()
	...
	var items []Book
	err := q.cache.GetWithTtl(qctx, "books:GetBookByNameMaybe:"+hashIfLong(fmt.Sprintf("%+v", ptrStr(name))), &items, dbRead, false, false)
	if err != nil {
		return nil, err
	}
...
func (q *Queries) GetBookBySpec(ctx context.Context, arg GetBookBySpecParams) ([]Book, error) {
	...
	dbRead := func() (any, time.Duration, error) {
		cacheDuration := time.Duration(time.Millisecond * 600000)
		rows, err := q.db.WQuery(qctx, "books.GetBookBySpec", getBookBySpec, arg.Name, arg.Price, arg.Dummy)
		if err != nil {
			return []Book(nil), 0, err
		}
		defer rows.Close()
...
func (q *Queries) InsertAndReturnID(ctx context.Context, arg InsertAndReturnIDParams) (*int32, error) {
	qctx, cancel := context.WithTimeout(ctx, time.Millisecond*250)
	defer cancel()
	row := q.db.WQueryRow(qctx, "books.InsertAndReturnID", insertAndReturnID,
		arg.Name,
		arg.Description,
		arg.Metadata,
		arg.Category,
...
func (q *Queries) ListByCategory(ctx context.Context, arg ListByCategoryParams) ([]Book, error) {
	...
	rows, err := q.db.WQuery(qctx, "books.ListByCategory", listByCategory, arg.Category, arg.After, arg.First)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (q *Queries) SearchBooks(ctx context.Context, name string) ([]Book, error) {
	...
	dbRead := func() (any, time.Duration, error) {
		cacheDuration := time.Duration(time.Millisecond * 600000)
		rows, err := q.db.WQuery(qctx, "books.SearchBooks", searchBooks, name)
		if err != nil {
			return []Book(nil), 0, err
		}
		defer rows.Close()
	...
	var items []Book
	err := q.cache.GetWithTtl(qctx, "books:SearchBooks:"+hashIfLong(fmt.Sprintf("%+v", name)), &items, dbRead, false, false)
	if err != nil {
		return nil, err
	}
...
func (q *Queries) SimpleCachedQuery(ctx context.Context) ([]Book, error) {
	qctx, cancel := context.WithTimeout(ctx, time.Millisecond*250)
	defer cancel()
	q.db.CountIntent("books.SimpleCachedQuery")
	dbRead := func() (any, time.Duration, error) {
		cacheDuration := time.Duration(time.Millisecond * 600000)
		rows, err := q.db.WQuery(qctx, "books.SimpleCachedQuery", simpleCachedQuery)
		if err != nil {
			return []Book(nil), 0, err
		}
		defer rows.Close()
	...
	var items []Book
	err := q.cache.GetWithTtl(qctx, "books:SimpleCachedQuery:", &items, dbRead, false, false)
	if err != nil {
		return nil, err
	}
