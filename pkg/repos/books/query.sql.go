...
func (q *Queries) Insert(ctx context.Context, arg InsertParams) error {
	_, err := q.db.WExec(ctx, "Insert", insert,
		arg.Name,
		arg.Description,
		arg.Metadata,
		arg.Category,
...
func (q *Queries) PartialUpdateByID(ctx context.Context, arg PartialUpdateByIDParams, getBookByID *int32) error {
...
func (q *Queries) SearchBooks(ctx context.Context, name string) ([]Book, error) {
	dbRead := func() (any, time.Duration, error) {
		cacheDuration := time.Duration(time.Millisecond * 600000)
		rows, err := q.db.WQuery(ctx, "SearchBooks", searchBooks, name)
		if err != nil {
			return nil, 0, err
		}
		defer rows.Close()
	...
	var items []Book
	err := q.cache.GetWithTtl(ctx, fmt.Sprintf("books:SearchBooks:%+v", name), &items, dbRead, false, false)
	if err != nil {
		return nil, err
	}
